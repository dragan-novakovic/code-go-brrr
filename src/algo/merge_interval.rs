pub fn merge_intervals(intervals: Vec<Vec<i32>>) -> Vec<Vec<i32>> {
    ...
    let sorted_intervals = intervals
        ...
        .fold(vec![] as Vec<Vec<i32>>, |mut acc, inter| {
            ...
            for index in 0..interval_length {
                if inter[0] < acc[index][0] {
                    acc.insert(index, inter.clone());
                    return acc;
                }
            }
    ...
    let clean_intervals = sorted_intervals.iter().enumerate().fold(
        vec![] as Vec<Vec<i32>>,
        |mut acc, (_index, interval)| {
            let acc_len = acc.len();
